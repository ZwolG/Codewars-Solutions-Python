...
def isbn_converter(isbn):
    ...
    prefix = list('978-')
    isbn_list.pop()
    twelve_digits = prefix + isbn_list
    twelve = [int(x) for x in twelve_digits if x.isnumeric()]
    # check digit
    
    new_arr = []
    for i, v in enumerate(twelve):
    ...
    twelve_digits.append(str(check_digit))

    return ''.join(twelve_digits)
